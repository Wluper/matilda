...
def handle_database_resource(id=None, user=None, annotationRate=None, DBcollection=None):
    ...
    if user:
        if request.method == "PUT":
            responseObject = DatabaseManagement.storeAnnotations( user, dialogueFile.activeCollection[user], annotationRate )
...
def handle_recover_request(user, collection):
    ...
    doc = DatabaseManagement.readDatabase("dialogues_collections","id",collection)
...
def handle_backup_resource(user, annotationRate):
    ...
    if request.method == "PUT":
        responseObject = DatabaseManagement.storeAnnotations(user, dialogueFile.activeCollection[user], annotationRate, True)
...
def handle_users(user=None, userPass=None, email=None): 
    ...
    if request.method == "GET":
        ...
        responseObject = DatabaseManagement.readDatabase("users","userName",None,["userName","password","email","role"])
...
def handle_collections(id=None, user=None):
    if not id:
        ...
        if request.method == "GET":
            ...
            collectionNames = DatabaseManagement.readDatabase("dialogues_collections","id")
    ...
    if id:
        ...
        if id == "ids":
            if request.method == "GET":
                ...
                collectionNames = DatabaseManagement.readDatabase("dialogues_collections","id", None, ["id","assignedTo","status","lastUpdate"])
        ...
        if request.method == "POST":
            ...
            response = DatabaseManagement.createDoc(id, "dialogues_collections", values["json"])
